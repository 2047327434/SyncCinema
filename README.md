...
[快速开始](#quick-start) · [功能特性](#features) · [部署](#deployment) · [API 文档](#api-reference) · [贡献指南](CONTRIBUTING.md)
...
SyncCinema 尊重您的隐私权。以下说明我们如何处理您的数据。

### 我们收集的信息

| 数据类型 | 用途 | 存储位置 |
|---------|------|---------|
| 用户名和密码 | 账号认证 | 服务器数据库（密码使用 bcrypt 加密） |
| 头像和个性签名 | 用户资料展示 | 服务器数据库 |
| 聊天消息 | 房间聊天和私聊 | 服务器数据库（持久化） |
| 房间数据 | 房间创建和管理 | 服务器数据库（房间无人后自动删除） |
| 分享文件 | 聊天文件分享 | 服务器数据库（单文件最大 5MB） |
| 观看历史和收藏 | 快速访问房间 | 浏览器 localStorage |
...
- 我们不与第三方共享您的数据
- 我们不会在未经明确许可下访问您的麦克风或摄像头（WebRTC 语音/屏幕分享需要用户主动授权）

### 第三方服务

SyncCinema 可能嵌入 YouTube 和 Bilibili 的视频内容（iframe 方式）。这些平台拥有各自的隐私政策，SyncCinema 不会向它们发送您的个人数据。

### 数据保留与删除

- 房间数据在所有成员离开后自动删除
- 聊天记录按房间保留（内存最多 200 条，持久化最多 100 条）
- 账号数据可应要求由管理员删除
- 您可在浏览器中清除本地观看历史和收藏

### 自托管部署

如果您自行部署 SyncCinema，您即数据控制者，需负责服务器安全、JWT_SECRET/CORS 配置、用户数据合规管理和备份策略。

> 完整版隐私声明（含英文版）见 [PRIVACY.md](PRIVACY.md)
